...
public class AdditionFluidValueTypeWorldRenderer implements IValueTypeWorldRenderer {
    ...
    public void renderValue(TileEntityRendererDispatcher rendererDispatcher, IPartContainer partContainer,
        ...
        if (!fluidStack.isEmpty()) {
            ...
            float min = 0F - 16 - 12.5F;
            float max = 12.5F + 16 + 12.5F;
            ...
            // Stack size
            matrixStack.translate(7F + 8.5, 8.5F + 18.5F, 0.1F);
            String string = String.valueOf(fluidStack.getAmount());
            float scale = ((float) 5) / (float) rendererDispatcher.getFontRenderer().getStringWidth(string) * 3;

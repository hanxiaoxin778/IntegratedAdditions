...
public class AdditionIngredientsValueTypeWorldRenderer implements IValueTypeWorldRenderer {
    ...
    public static void renderGrid(TileEntityRendererDispatcher rendererDispatcher, IPartContainer partContainer,
        ...
        for (int i = 0; i < matrixRadius; i++) {
            for (int j = 0; j < matrixRadius; j++) {
                ...
                if (renderValue != null) {
                    matrixStack.push();
                    matrixStack.translate(j * HugeDisplayPartOverlayRenderer.MAX - 16 * 3 - 12.5F, i * HugeDisplayPartOverlayRenderer.MAX - 16 * 3 - 12.5F, 0);
                    // Call value renderer for each value
                    IValueTypeWorldRenderer renderer = HugeDisplayPartOverlayRenderer.RENDERS.get(renderValue.getType());
                    if (renderer == null) {
                        renderer = HugeDisplayPartOverlayRenderer.DEFAULT;
                    }
